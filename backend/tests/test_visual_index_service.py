...
def test_visual_embedding_service_splits_large_image_requests(tmp_path: Path, monkeypatch) -> None:
    image_paths = []
    for index in range(5):
        image_path = tmp_path / f"drawing-{index}.png"
    ...
    class Response:
        ...
        def json(self) -> dict[str, object]:
            return {
                ...
                "distance_metric": "COSINE",
                "data": [
                    {"index": index, "embedding": [float(len(batch_sizes)), float(index)]}
