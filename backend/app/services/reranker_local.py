...
class LocalCrossEncoderReranker:
    ...
    def __init__(self, model_name: str, device: str, batch_size: int) -> None:
        ...
        logger.info(
            "本地Reranker模型加载开始: loaded=%s path=%s device=%s batch_size=%s",
            False,
            self.model_path,
            self.device,
            self.batch_size,
        ...
        logger.info(
            "本地Reranker模型加载完成: loaded=%s path=%s backend=%s device=%s elapsed_ms=%s",
            self.is_loaded,
            self.model_path,
            self.backend_name,
            self.device,
    ...
    def predict(self, query: str, documents: list[str]) -> list[float]:
        ...
        logger.info(
            "本地Reranker调用完成: loaded=%s path=%s backend=%s device=%s count=%s batch_size=%s elapsed_ms=%s",
            self.is_loaded,
            self.model_path,
            self.backend_name,
            self.device,
    ...
    def _resolve_device(self, requested_device: str) -> str:
        normalized_device = (requested_device or "cpu").lower().strip()
...
def get_local_reranker(model_name: str, device: str, batch_size: int) -> LocalCrossEncoderReranker:
    ...
    with _LOCAL_RERANKER_CACHE_LOCK:
        ...
        if cached_reranker is not None:
            logger.info(
                "本地Reranker模型缓存命中: loaded=%s path=%s device=%s batch_size=%s",
        ...
        logger.info(
            "本地Reranker模型已写入单例缓存: loaded=%s path=%s device=%s batch_size=%s elapsed_ms=%s",
