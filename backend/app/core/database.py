...
def seed_sensitive_content(db: Session) -> None:
    ...
    for code, name, type_code, pattern, mask, priority in table_rule_defaults:
        if code not in existing_rules:
            db.add(SensitiveFilterRule(code=code, name=name, sensitive_type_code=type_code, match_type="table_column", pattern=pattern, context_keywords="[]", window_size=30, mask_text=mask, priority=priority, enabled=True, version=1))
