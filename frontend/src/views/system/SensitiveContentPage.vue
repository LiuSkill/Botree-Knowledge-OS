...
function matchTypeLabel(value: string): string { return ({ regex: '正则表达式', keyword: '关键词', keyword_window: '关键词窗口', table_column: '表格列', table_row: '表格行', table_cell: '表格单元格' } as Record<string, string>)[value] || value; }
      ...
      <t-tab-panel value="rules" label="敏感规则">
        <div class="tab-content">
          <t-form class="system-filter-form" layout="inline" label-align="left" label-width="auto">
            ...
            <t-form-item label="匹配方式"><t-select v-model="ruleFilters.match_type" class="filter-select" clearable placeholder="全部方式"><t-option label="正则表达式" value="regex" /><t-option label="关键词" value="keyword" /><t-option label="关键词窗口" value="keyword_window" /><t-option label="表格列" value="table_column" /><t-option label="表格行" value="table_row" /><t-option label="表格单元格" value="table_cell" /></t-select></t-form-item>
